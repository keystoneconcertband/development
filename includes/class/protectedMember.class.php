				...
				if($value !== '') {
					...
					if($pendingUser) {
						try {
							...
							mail('web@keystoneconcerband.com, jonathan.gillette@gmail.com','KCB Email Update','(pend) Add email: ' . $value, implode("\r\n", $notificationHeader));
						}
						catch(Exception $e) {
							$this->getKcb()->LogError($e->getMessage());
							return false;
				...
				foreach ($emailsToAdd as $value) {
					if($value !== "") {
						...
						try {
							...
							if(!$pendingUser) {
								mail('members-request@keystoneconcertband.com', '', $subscribeBody, implode("\r\n", $headerAdd));
								mail('web@keystoneconcerband.com, jonathan.gillette@gmail.com','KCB Email Update','Add email: ' . $value, implode("\r\n", $notificationHeader));
							}
				...
				foreach ($emailsToDel as $value) {
					if($value !== "") {
						...
						try {
							...
							mail('web@keystoneconcertband.com, jonathan.gillette@gmail.com','KCB Email Update','Remove email: ' . $value, implode("\r\n", $notificationHeader));
